...
## 全体の流れ
0. **設計原則の接地**（`_shared/game-design-principles.md` を読む）
...
## Step 0: 設計原則の接地
`.claude/skills/_shared/game-design-principles.md` を読む（**時間で腐らない設計原則の単一の正**＝モバイル前提/指1本ゲート/コアループ/オンボ/リテンション/人数可変/拡散/課金/スコープ/vibe）。
- ここはトレンド(市場データ)と違い**毎回必ず参照**する。9評価軸はこの原則の採点ビューなので整合させる。
- Step 2 の generator と Step 4 の critic に渡すプロンプトに、この原則の要点（特に「指1本で成立するか」「ゼロ人でも面白い→人がいると更に」「初回1回目でコアの面白さを離脱せず体験→翌日も引ける(first_session / 最重要)」「個人スコープの地雷」）を必ず織り込む。
  - 集客は当面「広告」主体（外部拡散もRobloxオーガニック露出も当てにしない）前提。よって「クリップ映え・外部拡散(virality)」は採点軸から外し、その比重を first_session（広告で来た一人を1回目で逃さず翌日また来させる）に置く。
...
- `trends`: Step 1 の要約
- `design_principles`: Step 0 の原則の要点（指1本/ゼロ人でも面白い/初回1回目でコアを離脱せず体験→翌日も引ける(first_session)/個人スコープの地雷 等）
...
- weighted_total が崩れていたら自分で再計算(重み: originality1.4, vibe0.8, fun1.4, accessibility1.0, monetization1.2, social0.9, solo0.8, feasibility1.2, first_session1.3 / 合計10.0・満点100)。**9観点**であることに注意。
- weighted_total 降順で順位付け。同点は fun+first_session+originality の合計で割る。
...
- `report.html`: 単一HTMLの見やすいレポート。CSSはインライン、外部依存なし。次を含める:
  ...
  - **ランキング表**: 順位 / タイトル / tagline / weighted_total / effort / 9観点スコア(first_session含む)。
  - **カード**: 上位から各アイデアの pitch・surprise・core_loop_30s・progression・social_hook・solo_ok・first_session・monetization・roblox_feasibility と、critic の verdict・biggest_risk・improvement、9観点スコアのバー表示。first_session は最重要軸なので目立たせる。
