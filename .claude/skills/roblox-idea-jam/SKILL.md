...
- ファイルがあり、その先頭の `調査日時:` が**今日から7日以内**なら、**再検索せず**その md を読んで `trends` 要約として使う。
- 無い、または7日より古い、または引数に `--trends` がある場合は **WebSearch** で次を調べる:
...
- 検索したら結果を `research/roblox-trends-<yyyymmdd-hhmmss>.md` に保存する。先頭に必ず:
  ```
...
- weighted_total が崩れていたら自分で再計算(重み: originality1.5, vibe1.0, fun1.5, accessibility1.2, monetization1.3, social1.2, solo1.0, feasibility1.3 / 満点100)。
- weighted_total 降順で順位付け。同点は originality+fun の合計で割る。
...
- `report.html`: 単一HTMLの見やすいレポート。CSSはインライン、外部依存なし。次を含める:
  ...
  - **ランキング表**: 順位 / タイトル / tagline / weighted_total / effort / 8観点スコア。
  - **カード**: 上位から各アイデアの pitch・surprise・core_loop_30s・progression・social_hook・solo_ok・monetization・roblox_feasibility と、critic の verdict・biggest_risk・improvement、観点スコアのバー表示。
